...
-- Add missing columns to workouts if they don't exist (for existing databases)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='workouts' AND column_name='external_id') THEN
    ALTER TABLE public.workouts ADD COLUMN external_id TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='workouts' AND column_name='synced_from') THEN
    ALTER TABLE public.workouts ADD COLUMN synced_from TEXT;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='workouts' AND column_name='sync_timestamp') THEN
    ALTER TABLE public.workouts ADD COLUMN sync_timestamp TIMESTAMP WITH TIME ZONE;
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                 WHERE table_name='workouts' AND column_name='route_data') THEN
