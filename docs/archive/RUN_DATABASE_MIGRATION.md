...
1. Open: https://app.supabase.com/project/bdisppaxbvygsspcuymb/editor
...
CREATE INDEX IF NOT EXISTS idx_progression_plans_athlete ON progression_plans(athlete_id);
CREATE INDEX IF NOT EXISTS idx_progression_plans_status ON progression_plans(status);
CREATE INDEX IF NOT EXISTS idx_progression_plans_athlete_status ON progression_plans(athlete_id, status);

-- Add RLS policies
ALTER TABLE progression_plans ENABLE ROW LEVEL SECURITY;

-- Athletes can view their own plans
DROP POLICY IF EXISTS "Athletes can view own progression plans" ON progression_plans;
...
DROP POLICY IF EXISTS "Athletes can create own progression plans" ON progression_plans;
...
DROP POLICY IF EXISTS "Athletes can update own progression plans" ON progression_plans;
...
DROP TRIGGER IF EXISTS update_progression_plans_timestamp ON progression_plans;
...
```sql
SELECT COUNT(*) FROM progression_plans;
-- Should return 0 (empty table, ready for data)

SELECT * FROM pg_tables WHERE tablename = 'progression_plans';
...
Your database is now ready to store progression plans.
