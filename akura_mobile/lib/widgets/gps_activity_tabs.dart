...
class _GPSActivityTabsState extends State<GPSActivityTabs> with SingleTickerProviderStateMixin {
  ...
  Map<String, dynamic> _calculatePhysiologicalMetrics() {
    ...
    if (hrPercentage != null && hrPercentage >= 60 && hrPercentage <= 75) {
      mitochondrialStatus = 'Optimal zone for mitochondrial biogenesis (Zone 2-3). '
  ...
  List<Map<String, dynamic>> _generateSplits() {
    ...
    for (int i = 0; i < numSplits; i++) {
      ...
      splits.add({
        ...
        'pace': splitPace,
        'avgHR': hrForSplit,
        'elevation': elevPerKm > 0 ? (elevPerKm * (1 + math.sin(i * 0.7))).round() : 0,
      });
    }
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      DateFormat('EEE, MMM d • h:mm a').format(widget.workout.scheduledDate),
              ...
              Container(
                ...
                child: const Row(
                  ...
                  children: [
                    ...
                    SizedBox(width: 4),
                    Text('Completed', style: TextStyle(color: Colors.green, fontWeight: FontWeight.w600, fontSize: 11)),
                  ],
                ),
              ),
            ],
  ...
  Widget _buildOverviewTab() {
    ...
    final calories = widget.activity['calories'] as int?;
    final avgStrideLength = widget.activity['avg_stride_length'] as num?;
    final avgGroundContactTime = widget.activity['avg_ground_contact_time'] as num?;
    final avgVerticalOscillation = widget.activity['avg_vertical_oscillation'] as num?;
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                Row(
                  children: [
                    Expanded(child: _buildMetricCard('Distance', '${distanceKm.toStringAsFixed(2)} km', Icons.straighten, Colors.blue)),
                    const SizedBox(width: 12),
                    Expanded(child: _buildMetricCard('Time', '${durationMin.floor()}:${(durationSec % 60).toString().padLeft(2, '0')}', Icons.timer, Colors.orange)),
                  ],
                ),
                const SizedBox(height: 12),
                Row(
                  children: [
                    Expanded(child: _buildMetricCard('Avg Pace', avgPace != null ? '${avgPace.toStringAsFixed(2)} /km' : '-', Icons.speed, Colors.purple)),
                    const SizedBox(width: 12),
                    Expanded(child: _buildMetricCard('Elevation', elevGain != null ? '${elevGain.round()} m' : '-', Icons.terrain, Colors.green)),
                  ],
                ),
              ],
            ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('${avgHR.round()} bpm', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.red)),
                      ],
                    ),
                  ),
                  if (maxHR != null) ...[
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 4),
                          Text('${maxHR.round()} bpm', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.red)),
                        ],
                      ),
                    ),
                  ],
          ...
          if (avgCadence != null) ...[
            Row(
              children: [
                const Text('👟', style: TextStyle(fontSize: 20)),
                const SizedBox(width: 8),
                const Text('Cadence', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ],
            ),
            const SizedBox(height: 12),
            Container(
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text('Average', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
                        const SizedBox(height: 4),
                        Text('${avgCadence.round()} spm', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.orange)),
                      ],
                    ),
                  ),
                  if (maxCadence != null) ...[
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text('Maximum', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
                          const SizedBox(height: 4),
                          Text('${maxCadence.round()} spm', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.bold, color: Colors.orange)),
                        ],
                      ),
                    ),
                  ],
          ...
          if (avgStrideLength != null || avgGroundContactTime != null || avgVerticalOscillation != null) ...[
            Row(
              children: [
                const Text('📊', style: TextStyle(fontSize: 20)),
                const SizedBox(width: 8),
                const Text('Advanced Biomechanics', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ],
            ),
            const SizedBox(height: 12),
            Container(
              ...
              child: Column(
                children: [
                  if (avgStrideLength != null) _buildDetailRow('Stride Length', '${avgStrideLength.toStringAsFixed(2)} m', Icons.height),
                  if (avgGroundContactTime != null) _buildDetailRow('Ground Contact', '${avgGroundContactTime.round()} ms', Icons.timer_outlined),
                  if (avgVerticalOscillation != null) _buildDetailRow('Vertical Osc.', '${avgVerticalOscillation.toStringAsFixed(1)} cm', Icons.swap_vert),
                ],
              ),
            ),
            const SizedBox(height: 20),
          ...
          Container(
            ...
            child: Column(
              children: [
                if (maxSpeed != null) _buildDetailRow('Max Speed', '${maxSpeed.toStringAsFixed(2)} km/h', Icons.flash_on),
                if (calories != null) _buildDetailRow('Calories', '$calories kcal', Icons.local_fire_department),
              ],
            ),
          ),
  ...
  Widget _buildSplitsTab(List<Map<String, dynamic>> splits, double distanceKm, int durationSec) {
    return SingleChildScrollView(
      padding: const EdgeInsets.all(20),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Pace Trend', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          ...
          // Splits Table
          const Text('Splits Details', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      const SizedBox(width: 40, child: Text('KM', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                      const Expanded(child: Text('Time', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                      const Expanded(child: Text('Pace', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                      SizedBox(width: 60, child: Text('Avg HR', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                    ],
                  ),
                ),
                    ...
                    child: Row(
                      children: [
                        ...
                        SizedBox(
                          width: 60,
                          child: hr != null
                              ? Text('$hr bpm', style: const TextStyle(fontSize: 14))
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                const Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Physiological Analysis', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  ],
                ),
                const SizedBox(height: 12),
                if (analysis['fitnessLevel'] != null) ...[
                  _buildAnalysisRow('Fitness Level', analysis['fitnessLevel'], Icons.fitness_center),
                  const SizedBox(height: 8),
                ],
                if (analysis['vo2max'] != null) ...[
                  _buildAnalysisRow('VO2max Estimate', '${(analysis['vo2max'] as double).toStringAsFixed(1)} ml/kg/min', Icons.air),
                  const SizedBox(height: 8),
                ],
                if (analysis['cardiacOutput'] != null) ...[
                  _buildAnalysisRow('Cardiac Output', '${(analysis['cardiacOutput'] as double).toStringAsFixed(1)} L/min', Icons.favorite),
                  const SizedBox(height: 8),
                ],
                if (analysis['hrZone'] != null) ...[
                  _buildAnalysisRow('Heart Rate Zone', analysis['hrZone'], Icons.monitor_heart),
                  const SizedBox(height: 8),
                ],
                if (analysis['metabolicEfficiency'] != null) ...[
                  _buildAnalysisRow('Metabolic Efficiency', '${(analysis['metabolicEfficiency'] as double).toStringAsFixed(1)} kcal/km', Icons.local_fire_department),
                ],
              ],
            ),
          ),
                ...
                children: [
                  const Row(
                    children: [
                      ...
                      SizedBox(width: 8),
                      Text('Mitochondrial Adaptation', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Text(
          ...
          const Text('Pace Analysis', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          Container(
            height: 220,
            padding: const EdgeInsets.all(16),
                ...
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Average', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                        Text(
                          '${(splits.map((s) => s['pace'] as double).reduce((a, b) => a + b) / splits.length).toStringAsFixed(2)}',
                          style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.purple),
                        ),
                        const Text('min/km', style: TextStyle(fontSize: 11, color: Colors.grey)),
                      ],
                    ),
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.end,
                      children: [
                        const Text('Best Split', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12)),
                        Text(
                          '${splits.map((s) => s['pace'] as double).reduce((a, b) => a < b ? a : b).toStringAsFixed(2)}',
          ...
          // HR Chart with zones
          if (avgHR != null) ...[
            const Text('Heart Rate Analysis', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            Container(
              height: 220,
              padding: const EdgeInsets.all(16),
              ...
              child: Column(
                children: [
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Average', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                            Text('${avgHR.round()} bpm', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.red)),
                          ],
                        ),
                      ),
                      if (maxHR != null)
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text('Maximum', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                              Text('${maxHR.round()} bpm', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.red)),
                            ],
                          ),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text('${(analysis['hrPercentage'] as double).toStringAsFixed(0)}%', 
                                style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, 
                                  color: (analysis['hrPercentage'] as double) > 85 ? Colors.orange : Colors.red)),
                            ],
                          ),
                        ),
                    ],
                  ...
                  Expanded(
                    child: CustomPaint(
                      painter: DetailedHRChartPainter(splits, maxHR?.toDouble()),
                      child: Container(),
                    ),
                  ),
                ],
          ...
          Container(
            ...
            child: Row(
              children: [
                Icon(
                  (analysis['isEconomical'] == true) ? Icons.check_circle : Icons.warning,
                  color: (analysis['isEconomical'] == true) ? Colors.green : Colors.orange,
                  size: 32,
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        (analysis['isEconomical'] == true) ? 'Running Economy: Optimal' : 'Running Economy: Needs Attention',
                        style: TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 16,
                          color: (analysis['isEconomical'] == true) ? Colors.green[900] : Colors.orange[900],
                        ),
                      ),
                      const SizedBox(height: 4),
                      Text(
                ...
                children: [
                  const Row(
                    children: [
                      ...
                      SizedBox(width: 8),
                      Text('Coaching Recommendations', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    ],
                  ),
                  const SizedBox(height: 16),
                  ...(analysis['coachingRecommendations'] as List).asMap().entries.map((entry) {
                    return Padding(
      ...
      child: Column(
        children: [
          ...
          FittedBox(
            ...
            child: Text(value, style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: color), textAlign: TextAlign.center),
          ),
        ],
      ),
    );
...
class PaceChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < splits.length; i++) {
      ...
      final y = size.height - ((pace - minPace) / (maxPace - minPace)) * size.height;
...
class HRChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final hrValues = splits.map((s) => s['avgHR'] as int?).where((hr) => hr != null).map((hr) => hr!.toDouble()).toList();
...
class DetailedHRChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < splits.length; i++) {
      ...
      final x = (validIndex / (hrValues.length - 1)) * size.width;
      final y = size.height - ((hr - hrMin) / (hrMax - hrMin)) * size.height;
      if (validIndex == 0) linePath.moveTo(x, y);
      else linePath.lineTo(x, y);
      canvas.drawCircle(Offset(x, y), 5, Paint()..color = Colors.white);
      canvas.drawCircle(Offset(x, y), 3.5, Paint()..color = Colors.red);
      validIndex++;
    }
    canvas.drawPath(linePath, Paint()..color = Colors.red..strokeWidth = 2.5..style = PaintingStyle.stroke..strokeCap = StrokeCap.round);
  }
