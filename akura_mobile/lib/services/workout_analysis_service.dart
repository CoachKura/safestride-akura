...
class WorkoutAnalysisService {
  ...
  static WorkoutAnalysis analyzeWorkoutData({
    ...
    if (avgCadence < 160) {
      issues.add(AnalysisIssue(
        ...
        problem: 'Your cadence of ${avgCadence.toStringAsFixed(0)} spm is significantly below optimal range. '
            'Low cadence increases ground contact time, leading to higher impact forces on joints.',
        why: 'When stride rate is too low, you spend more time on the ground with each step, '
        ...
        remedy: '1. Practice high-cadence drills: 30-second intervals at 180+ spm\n'
        ...
        protocolFocus: ['Cadence Drills', 'Plyometric Exercises', 'Rhythm Training'],
      ));
      ...
      issues.add(AnalysisIssue(
        ...
        currentValue: '${avgCadence.toStringAsFixed(0)} spm',
        targetValue: '170-180 spm',
        problem: 'Your cadence is below the optimal range for injury prevention.',
        why: 'Elite runners maintain 170-180 spm to minimize impact forces and improve efficiency.',
        remedy: 'Gradually increase cadence through focused drills and metronome training.',
        protocolFocus: ['Cadence Awareness Drills'],
      ));
    } else if (avgCadence >= 170 && avgCadence <= 185) {
      strengths.add('✅ Excellent cadence (${avgCadence.toStringAsFixed(0)} spm) - optimal for injury prevention');
    }
    ...
    if (verticalOscillation > 10.0) {
      issues.add(AnalysisIssue(
        ...
        targetValue: '6-8 cm',
        problem: 'Your vertical oscillation (bounce) of ${verticalOscillation.toStringAsFixed(1)} cm is excessive. '
            'You are wasting significant energy moving up and down instead of forward.',
        why: 'High vertical oscillation (>10cm) indicates poor running economy and increased impact loading. '
        ...
        remedy: '1. Core strengthening: Planks, dead bugs, bird dogs (3x/week)\n'
        ...
        protocolFocus: ['Core Stability', 'Running Form Correction', 'Glute Strengthening'],
      ));
      ...
      strengths.add('✅ Optimal vertical oscillation (${verticalOscillation.toStringAsFixed(1)} cm) - excellent form');
    }
    ...
    if (groundContactTime > 280) {
      issues.add(AnalysisIssue(
        ...
        problem: 'Your foot stays on the ground too long (${groundContactTime.toStringAsFixed(0)} ms), '
        ...
        protocolFocus: ['Plyometrics', 'Calf Strengthening', 'Reactive Strength'],
      ));
      ...
      issues.add(AnalysisIssue(
        ...
        why: 'Improving reactive strength will enhance performance and reduce injury risk.',
      ...
      strengths.add('✅ Excellent ground contact time (${groundContactTime.toStringAsFixed(0)} ms)');
    }
    ...
    if (trainingLoad > 500 && recoveryScore < 60) {
      issues.add(AnalysisIssue(
        ...
        problem: 'High training load with poor recovery is a red flag for overtraining and injury.',
      ...
      issues.add(AnalysisIssue(
        ...
        targetValue: '70-85%',
        problem: 'Your recovery score is not keeping pace with training demands.',
        why: 'Suboptimal recovery reduces adaptation and increases injury risk.',
    ...
    if (weeklyDistance > 60 && injuryRisk < 60) {
      issues.add(AnalysisIssue(
        ...
        problem: 'Your weekly distance combined with low AISRI score indicates overload.',
        why: 'High volume without adequate structural readiness leads to overuse injuries.',
        remedy: 'Reduce weekly volume by 10-15% until AISRI score improves above 65.',
        protocolFocus: ['Volume Reduction', 'Strengthening'],
      ));
    } else if (weeklyDistance >= 30 && weeklyDistance <= 60) {
      strengths.add('✅ Appropriate weekly distance (${weeklyDistance.toStringAsFixed(1)} km)');
    }
    ...
    if (avgHeartRate > 0) {
      ...
      if (hrPercentage > 85) {
        issues.add(AnalysisIssue(
          ...
          currentValue: '${avgHeartRate.toStringAsFixed(0)} bpm (${hrPercentage.toStringAsFixed(0)}% max)',
          targetValue: '70-80% max HR for base building',
          problem: 'You are training too hard too often.',
          why: 'Constantly high heart rate indicates inadequate easy running, leading to burnout.',
          remedy: 'Follow 80/20 rule: 80% easy (conversational pace), 20% hard efforts.',
  ...
  static double _calculateAverageCadence(List<Map<String, dynamic>> activities) {
  ...
  static double _calculateAverageHeartRate(List<Map<String, dynamic>> activities) {
    if (activities.isEmpty) return 0;
  ...
  static double _calculateWeeklyDistance(List<Map<String, dynamic>> activities) {
    if (activities.isEmpty) return 0;
  ...
  static double _calculateVerticalOscillation(List<Map<String, dynamic>> activities) {
  ...
  static double _calculateGroundContactTime(List<Map<String, dynamic>> activities) {
  ...
  static int _calculateOverallRisk(List<AnalysisIssue> issues, int baseInjuryRisk) {
    int criticalCount = issues.where((i) => i.severity == IssueSeverity.critical).length;
    int warningCount = issues.where((i) => i.severity == IssueSeverity.warning).length;
  ...
  static List<String> _generateRecommendations(List<AnalysisIssue> issues) {
    ...
    if (criticalIssues.isNotEmpty) {
      recommendations.add('🚨 URGENT: Address ${criticalIssues.length} critical issue(s) immediately');
