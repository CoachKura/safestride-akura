...
class GPSActivity {
  ...
  factory GPSActivity.fromJson(Map<String, dynamic> json) {
    return GPSActivity(
      id: json['id'],
      platform: GPSPlatform.values.firstWhere((e) => e.name == json['platform']),
...
class GPSDataFetcher {
      ...
      for (var activity in data) {
        if (activity['activityType'] == 'RUNNING' || 
          ...
          activities.add(GPSActivity(
            ...
            avgVerticalOscillation: activity['avgVerticalOscillation']?.toDouble(),
  ...
  Future<List<GPSActivity>> _fetchCorosActivities(
    ...
    final queryParams = <String, String>{
      if (startDate != null) 'start_date': (startDate.millisecondsSinceEpoch ~/ 1000).toString(),
      if (endDate != null) 'end_date': (endDate.millisecondsSinceEpoch ~/ 1000).toString(),
      if (limit != null) 'limit': limit.toString(),
    };
  ...
  List<GPSActivity> _parseCorosActivities(dynamic data) {
    ...
    if (data['data'] != null && data['data']['dataList'] is List) {
      for (var activity in data['data']['dataList']) {
        if (activity['mode'] == 11) { // 11 = Running
          activities.add(GPSActivity(
            id: activity['labelId'].toString(),
            platform: GPSPlatform.coros,
            startTime: DateTime.fromMillisecondsSinceEpoch(activity['startTime'] * 1000),
            durationSeconds: activity['duration'] as int,
            distanceMeters: (activity['distance'] as num).toDouble(),
  ...
  Future<List<GPSActivity>> _fetchStravaActivities(
    ...
    try {
      while (hasMoreActivities) {
        ...
        if (startDate != null) {
          queryParams['after'] = (startDate.millisecondsSinceEpoch ~/ 1000).toString();
        }
        if (endDate != null) {
      ...
      for (var activity in data) {
        if (activity['type'] == 'Run' || activity['type'] == 'TrailRun') {
          ...
          activities.add(GPSActivity(
            ...
            elevationLoss: (activity['total_elevation_loss'] as num?)?.toDouble(),
  ...
  Future<Map<String, dynamic>> saveActivitiesToDatabase(List<GPSActivity> activities) async {
    final userId = _supabase.auth.currentUser?.id;
    if (userId == null) throw Exception('User not logged in');
  ...
  Future<Map<DateTime, List<GPSActivity>>> getActivitiesByDateRange({
    ...
    try {
      ...
      final response = platform != null
          ? await baseQuery.eq('platform', platform.name).order('start_time', ascending: false)
      ...
      for (var row in response) {
        activities.add(GPSActivity(
          ...
          platform: GPSPlatform.values.firstWhere((e) => e.name == row['platform']),
  ...
  Future<ActivityDetails> _fetchStravaActivityDetails(String activityId, String accessToken) async {
    try {
  ...
  List<DataPoint> _parseStravaStreams(Map<String, dynamic> streams, String startDateStr) {
  ...
  Future<ActivityDetails> _fetchCorosActivityDetails(String activityId, String accessToken) async {
...
class VO2DataAdapter {
  ...
  static Map<String, dynamic> convertVO2ToAISRI({
    ...
    final aisriScore = ((enduranceScore + mobilityScore + strengthScore) / 3).round();
