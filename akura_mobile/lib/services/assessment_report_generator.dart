...
class ExecutiveSummary {
  ...
  String generateSummary() {
    ...
    summary.writeln('⏱️ ESTIMATED RECOVERY TIME: $estimatedRecoveryWeeks weeks\n');
    
    summary.writeln('📈 PILLAR BREAKDOWN:');
    pillarScores.forEach((pillar, score) {
      String status = score >= 80 ? '✅' : score >= 60 ? '🟡' : '🔴';
...
class CurrentCondition {
  ...
  String generateConditionReport() {
    ...
    romAssessments.forEach((testName, assessment) {
      ...
      report.writeln('   Your Result: ${assessment.yourValue}${assessment.unit}');
    ...
    String fatigueLevel = perceivedFatigue <= 3 ? 'Low' : 
                          perceivedFatigue <= 6 ? 'Moderate' : 'High';
    report.writeln('   • Perceived Fatigue: $perceivedFatigue/10 ($fatigueLevel)');
...
class GaitPathology {
  ...
  String generatePathologyReport() {
    ...
    report.writeln('🔍 $typeName ($severity - ${(confidenceLevel * 100).toStringAsFixed(0)}% confidence)\n');
  ...
  String _getPathologyTypeName(String type) {
    switch (type) {
      case 'bow_legs': return 'Bow Legs (Genu Varum)';
      case 'knock_knees': return 'Knock Knees (Genu Valgum)';
      case 'overpronation': return 'Overpronation';
      case 'underpronation': return 'Underpronation (Supination)';
      default: return type;
    }
  }
}
...
class RecoveryRoadmap {
  ...
  String generateRoadmapReport() {
    ...
    for (var phase in phases) {
      report.writeln('${phase.phaseNumber}. ${phase.phaseName.toUpperCase()} (Weeks ${phase.weekRange})');
...
class AssessmentReportGenerator {
  ...
  static AssessmentReport generateReport({
    ...
    final executiveSummary = _buildExecutiveSummary(
      ...
      goalStatement: goalStatement ?? assessmentData['goals'] ?? 'Improve running performance and reduce injury risk',
    );
    ...
    final recoveryRoadmap = _buildRecoveryRoadmap(
      ...
      goalStatement: goalStatement ?? assessmentData['goals'] ?? 'Improve running performance',
    );
  ...
  static CurrentCondition _buildCurrentCondition(Map<String, dynamic> a) {
    ...
    romAssessments['Ankle Dorsiflexion'] = ROMAssessment(
      ...
      unit: 'cm',
      normalRange: '9-12 cm',
      severity: ankleDorsi < 7 ? 'Critical' : ankleDorsi < 9 ? 'High' : ankleDorsi < 12 ? 'Moderate' : 'Normal',
    ...
    romAssessments['Hip Flexion'] = ROMAssessment(
      ...
      unit: '°',
      normalRange: '110-130°',
      severity: hipFlexion < 100 ? 'Critical' : hipFlexion < 110 ? 'High' : hipFlexion < 120 ? 'Moderate' : 'Normal',
      deficitPercentage: hipFlexion < 110 ? ((110 - hipFlexion) / 110 * 100) : null,
    );
    ...
    Map<String, String> qualitativeAssessments = {
      'Knee Extension Strength': a['knee_extension_strength'] ?? 'Moderate (45-90°)',
      'Shoulder Internal Rotation': a['shoulder_internal_rotation'] ?? 'Mid-back',
  ...
  static RecoveryRoadmap _buildRecoveryRoadmap({
    ...
    int criticalCount = injuryRisks.where((r) => r.severity == 'Critical').length;
    int highCount = injuryRisks.where((r) => r.severity == 'High').length;
    
    if (criticalCount >= 3 || gaitPathologies.where((gp) => gp.severity == 'Severe').length >= 2) {
      totalWeeks = 16;
    ...
    phases.add(RoadmapPhase(
      ...
      weekRange: '${(totalWeeks / 2).ceil() + 1}-${(totalWeeks * 3 / 4).ceil()}',
      goals: [
  ...
  static List<Milestone> _buildMilestones({
    ...
    milestones.add(Milestone(
      ...
      protocolName: phases.length > 1 ? phases[1].protocolName : 'Foundation Protocol',
    ...
    milestones.add(Milestone(
      ...
      protocolName: phases.length > 2 ? phases[2].protocolName : 'Functional Strength Protocol',
    ...
    milestones.add(Milestone(
      ...
      protocolName: phases.length > 3 ? phases[3].protocolName : 'Performance Integration Protocol',
  ...
  static String _buildNextStepsRecommendation({
    ...
    if (gaitPathologies.isNotEmpty) {
      ...
      steps.writeln('   • Review footwear recommendations in Gait Analysis section');
    ...
    if (injuryRisks.where((r) => r.severity == 'Critical').isNotEmpty) {
      steps.writeln('⚠️ CRITICAL: Consider consulting a sports medicine professional');
      steps.writeln('   Your assessment indicates high injury risk in critical areas.');
      steps.writeln('   A physical therapist can provide hands-on assessment and treatment.\n');
    }
