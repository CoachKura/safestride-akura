...
class WorkoutService {
  ...
  Future<List<StructuredWorkout>> getUpcomingWorkouts({int limit = 7}) async {
    ...
    return workouts
        .where((w) => w.scheduledDate != null && !w.scheduledDate!.isBefore(today))
        .toList()
  ...
  Future<StructuredWorkout> createFromTemplate(StructuredWorkout template, {DateTime? scheduledDate}) async {
  ...
  // Duplicate workout
  Future<StructuredWorkout> duplicateWorkout(StructuredWorkout workout, {DateTime? newDate}) async {
  ...
  Future<void> _saveAll(List<StructuredWorkout> workouts) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_workoutsKey, jsonEncode(workouts.map((w) => w.toJson()).toList()));
  }
  ...
  List<StructuredWorkout> _getDefaultTemplates() {
    return [
      ...
      StructuredWorkout(
        ...
        isTemplate: true,
        steps: [
          WorkoutStep(type: StepType.warmUp, target: StepTarget.duration, targetValue: 300, order: 0),
          WorkoutStep(type: StepType.run, target: StepTarget.distance, targetValue: 5000, targetUnit: 'km', order: 1),
          WorkoutStep(type: StepType.coolDown, target: StepTarget.duration, targetValue: 300, order: 2),
        ],
      ),
      ...
      StructuredWorkout(
        ...
        isTemplate: true,
        steps: [
          WorkoutStep(type: StepType.warmUp, target: StepTarget.duration, targetValue: 600, order: 0),
          WorkoutStep(type: StepType.repeat, repeatCount: 5, repeatSteps: [
            WorkoutStep(type: StepType.interval, target: StepTarget.distance, targetValue: 1000, targetUnit: 'm', order: 0),
            WorkoutStep(type: StepType.recovery, target: StepTarget.duration, targetValue: 120, order: 1),
          ], order: 1),
          WorkoutStep(type: StepType.coolDown, target: StepTarget.duration, targetValue: 600, order: 2),
        ],
      ),
      ...
      StructuredWorkout(
        ...
        steps: [
          WorkoutStep(type: StepType.warmUp, target: StepTarget.duration, targetValue: 600, order: 0),
          WorkoutStep(type: StepType.run, target: StepTarget.duration, targetValue: 1200, targetHRZone: 4, order: 1),
          WorkoutStep(type: StepType.coolDown, target: StepTarget.duration, targetValue: 600, order: 2),
        ],
      ),
      ...
      StructuredWorkout(
        ...
        steps: [
          ...
          WorkoutStep(type: StepType.run, target: StepTarget.distance, targetValue: 16000, targetUnit: 'km', targetHRZone: 2, order: 1),
          WorkoutStep(type: StepType.coolDown, target: StepTarget.open, order: 2),
        ],
      ),
      ...
      StructuredWorkout(
        ...
        steps: [
          WorkoutStep(type: StepType.warmUp, target: StepTarget.duration, targetValue: 600, order: 0),
          WorkoutStep(type: StepType.repeat, repeatCount: 8, repeatSteps: [
            WorkoutStep(type: StepType.interval, target: StepTarget.distance, targetValue: 400, targetUnit: 'm', order: 0),
            WorkoutStep(type: StepType.recovery, target: StepTarget.duration, targetValue: 90, order: 1),
          ], order: 1),
          WorkoutStep(type: StepType.coolDown, target: StepTarget.duration, targetValue: 600, order: 2),
        ],
      ),
      ...
      StructuredWorkout(
        ...
        steps: [
          WorkoutStep(type: StepType.warmUp, target: StepTarget.duration, targetValue: 600, order: 0),
          WorkoutStep(type: StepType.repeat, repeatCount: 4, repeatSteps: [
            WorkoutStep(type: StepType.interval, target: StepTarget.duration, targetValue: 180, order: 0),
            WorkoutStep(type: StepType.recovery, target: StepTarget.duration, targetValue: 120, order: 1),
            WorkoutStep(type: StepType.interval, target: StepTarget.duration, targetValue: 90, order: 2),
            WorkoutStep(type: StepType.recovery, target: StepTarget.duration, targetValue: 60, order: 3),
          ], order: 1),
          WorkoutStep(type: StepType.coolDown, target: StepTarget.duration, targetValue: 600, order: 2),
        ],
      ),
  ...
  Future<void> syncFromCloud() async {
    ...
    try {
      ...
      for (final w in cloudWorkouts) {
        ...
        if (!merged.containsKey(w.id) || w.createdAt.isAfter(merged[w.id]!.createdAt)) {
          merged[w.id] = w;
        }
      }
