...
class CalendarScheduler {
    ...
    try {
      ...
      for (var workout in protocol.workouts) {
        try {
          ...
          final daysFromStart = ((workout.week - 1) * 7) + ((workout.dayOfWeek - 1) * 2); // Every other day
  ...
  Future<String> _createProtocolRecord(String athleteId, GeneratedProtocol protocol) async {
    ...
    final response = await _supabase
        ...
        .insert({
          ...
          'target_injury': protocol.injuryRisk == 'high' ? 'High injury risk' : null,
          'target_deficit': protocol.focusAreas.join(', '),
          'expected_outcomes': 'Improved ${protocol.focusAreas.join(", ")} performance',
          'is_template': false,
        })
        .select('id')
        .single();
    ...
    try {
      ...
      final newCoach = await _supabase
          ...
          .insert({
            ...
            'bio': 'Automated protocol generation based on Strava data and AISRI assessment',
          })
          .select('id')
          .single();
