    ...
    try {
      ...
      if (userId != null) {
        try {
          ...
          if (gpsActivities != null && gpsActivities.isNotEmpty) {
  ...
  List<WorkoutCalendarEntry> _getMockPastWorkouts(DateTime month) {
    ...
    for (int i = 14; i >= 0; i--) {
      ...
      if (date.month == month.month) {
        ...
        if (i % 2 == 0) {
          final status = i > 1 ? 'completed' : (i == 1 ? 'completed' : 'scheduled');
          mockWorkouts.add(WorkoutCalendarEntry(
            ...
            workout: Workout(
              id: 'mock-workout-$i',
              workoutName: i % 6 == 0 ? 'Mobility & Recovery' : (i % 4 == 0 ? 'Strength Training' : 'Balance & Prevention'),
  ...
  WorkoutCalendarEntry? _getMockTodayWorkout() {
    ...
    return WorkoutCalendarEntry(
      ...
      workout: Workout(
        ...
        workoutType: 'injury_prevention',
        exercises: [
          Exercise(name: 'Cadence Drills', sets: 4, reps: 10, notes: 'High knees running in place, focus on quick foot turnover (180 spm)'),
