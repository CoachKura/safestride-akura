...
class StravaProtocolService {
  ...
  Future<ProtocolGenerationResult> generateAndScheduleProtocol({
    ...
    try {
      ...
      if (activities.isEmpty) {
        throw Exception('No Strava activities found. Please sync your Strava account first.');
      }
  ...
  Future<List<Map<String, dynamic>>> _fetchStravaActivities(String athleteId) async {
    try {
  ...
  Future<List<Map<String, dynamic>>> _fetchActivitiesFromDatabase(String athleteId) async {
    try {
      final response = await _supabase
          .from('gps_activities')
          .select()
          .eq('athlete_id', athleteId)
          .gte('start_time', DateTime.now().subtract(const Duration(days: 30)).toIso8601String())
    ...
    return [
      {
        ...
        'average_cadence': 75, // half cadence (151 spm actual - matches V.O2 data)
...
class ProtocolGenerationResult {
  ...
  String get detailedSummary {
    ...
    if (analysis != null && analysis!.hasData) {
      parts.add('\n\n📊 Analysis:');
      parts.add('Cadence: ${analysis!.avgCadence.toStringAsFixed(0)} spm (${analysis!.cadenceStatus})');
