...
class TrainingPlanService {
  ...
  Future<AthleteProfile> analyzeAthleteProfile() async {
    ...
    final weeklyDistance = (totalDistance / 4) / 1000; // km per week (30 days ≈ 4 weeks)
    final avgPace = totalTime > 0 ? (totalTime / 60) / (totalDistance / 1000) : 0; // min/km
  ...
  List<WorkoutType?> _getWeeklySchedule(FitnessLevel level, TrainingGoal goal, bool isRecoveryWeek) {
  ...
  StructuredWorkout _createWorkout(WorkoutType type, AthleteProfile profile, double weekDistance, TrainingGoal goal) {
    final pacePerKm = profile.avgPace > 0 ? profile.avgPace : 6.0; // Default 6 min/km
  ...
  StructuredWorkout _createEasyRun(double basePace, double weekDistance) {
    ...
    final easyPace = basePace + 0.5; // 30s slower per km
    
    return StructuredWorkout(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      name: 'Easy Run',
      description: 'Comfortable aerobic pace. Should be able to hold a conversation.',
      steps: [
        WorkoutStep(
  ...
  StructuredWorkout _createTempoRun(double basePace, double weekDistance) {
    final tempoPace = basePace - 0.5; // 30s faster per km
    final tempoDistance = (weekDistance * 0.12).clamp(3.0, 8.0) * 1000;
    
    return StructuredWorkout(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      name: 'Tempo Run',
      description: 'Comfortably hard pace. Challenging but sustainable for 20-40 minutes.',
      steps: [
        WorkoutStep(
          id: '${DateTime.now().millisecondsSinceEpoch}_warmup',
          type: StepType.warmUp,
  ...
  StructuredWorkout _createIntervalWorkout(double basePace, TrainingGoal goal) {
    final intervalPace = basePace - 1.0; // 1 min faster per km
    int reps;
    ...
    return StructuredWorkout(
      ...
      description: '${reps}x${intervalDistance.toInt()}m at fast pace with recovery jogs.',
      steps: steps,
    );
  }

  StructuredWorkout _createLongRun(double basePace, double weekDistance, TrainingGoal goal) {
    ...
    final easyPace = basePace + 0.5;
    
    return StructuredWorkout(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      name: 'Long Run',
      description: 'Build endurance at comfortable pace. Focus on time on feet.',
      steps: [
        WorkoutStep(
          id: '${DateTime.now().millisecondsSinceEpoch}_warmup',
          type: StepType.warmUp,
  ...
  StructuredWorkout _createRecoveryRun(double basePace) {
    final recoveryPace = basePace + 1.0;
      ...
      for (final day in week.days) {
        if (day.workout != null && !day.isRestDay) {
          await _supabase.from('athlete_calendar').insert({
            ...
            'workout_type': day.workout!.name.toLowerCase().replaceAll(' ', '_'),
...
enum TrainingGoal { general5k, general10k, halfMarathon, marathon, speedImprovement, enduranceBase, maintenance }
enum WorkoutType { easyRun, tempo, intervals, longRun, recovery, fartlek, hills }
...
class TrainingPlan {
  ...
  double get totalDistance => weeks.fold(0.0, (sum, w) => sum + w.totalDistance);
  int get totalWorkouts => weeks.fold(0, (sum, w) => sum + w.days.where((d) => !d.isRestDay).length);
}
