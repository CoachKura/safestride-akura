...
class ProtocolGenerator {
  ...
  // Fetch exercises from database based on focus areas
  Future<List<Map<String, dynamic>>> _fetchRelevantExercises(List<String> focusAreas) async {
    try {
  ...
  List<Map<String, dynamic>> _selectExercises({
    ...
    if (filtered.length < count) {
      final remaining = exercises.where((ex) => !filtered.contains(ex)).toList();
  ...
  String _generateDescription(StravaAnalysis analysis, List<String> focusAreas) {
    final parts = <String>[];
    
    parts.add('Personalized protocol based on your Strava data and AISRI assessment.');
    
    if (analysis.avgCadence > 0) {
      parts.add('Your average cadence is ${analysis.avgCadence.toStringAsFixed(0)} spm.');
    }
