...
class RepeatingGroup extends WorkoutSet {
  ...
  @override
  String get description {
    final setsStr = sets.map((s) => s.description).join(', ');
    return ' x ()';
  }
...
class QualitySessionWorkout {
  ...
  double get totalDistance {
    ...
    // Add warmup
    if (warmupUnit == WorkoutUnit.kilometers || warmupUnit == WorkoutUnit.miles || warmupUnit == WorkoutUnit.meters) {
      total += warmup;
    }
    
    // Add cooldown
    if (cooldownUnit == WorkoutUnit.kilometers || cooldownUnit == WorkoutUnit.miles || cooldownUnit == WorkoutUnit.meters) {
      total += cooldown;
    }
