...
class WorkoutStep {
  ...
  String get typeName {
    switch (type) {
      case StepType.warmUp: return 'Warm up';
      case StepType.run: return 'Run';
      case StepType.walk: return 'Walk';
      case StepType.interval: return 'Interval';
      case StepType.recovery: return 'Recovery';
      case StepType.coolDown: return 'Cool down';
      case StepType.rest: return 'Rest';
      case StepType.repeat: return 'Repeat';
    }
  }
  ...
  Color get typeColor {
    switch (type) {
      case StepType.warmUp: return Colors.red;
      case StepType.run: return Colors.blue;
      case StepType.walk: return Colors.orange;
      case StepType.interval: return Colors.purple;
      case StepType.recovery: return Colors.green;
      case StepType.coolDown: return Colors.teal;
      case StepType.rest: return Colors.grey;
      case StepType.repeat: return Colors.amber;
    }
  }
  ...
  IconData get typeIcon {
    switch (type) {
      case StepType.warmUp: return Icons.whatshot;
      case StepType.run: return Icons.directions_run;
      case StepType.walk: return Icons.directions_walk;
      case StepType.interval: return Icons.flash_on;
      case StepType.recovery: return Icons.healing;
      case StepType.coolDown: return Icons.ac_unit;
      case StepType.rest: return Icons.pause;
      case StepType.repeat: return Icons.repeat;
    }
  }
  ...
  Duration get estimatedDuration {
    switch (target) {
      ...
      case StepTarget.distance:
        ...
        final avgPace = (targetPaceMin ?? 6) + ((targetPaceMax ?? 6) - (targetPaceMin ?? 6)) / 2;
  ...
  factory WorkoutStep.fromJson(Map<String, dynamic> json) {
    return WorkoutStep(
      ...
      repeatSteps: json['repeatSteps'] != null
          ? (json['repeatSteps'] as List).map((s) => WorkoutStep.fromJson(s)).toList()
          : null,
      notes: json['notes'],
      order: json['order'] ?? 0,
    );
...
class StructuredWorkout {
  ...
  factory StructuredWorkout.fromJson(Map<String, dynamic> json) {
    return StructuredWorkout(
      ...
      steps: (json['steps'] as List?)?.map((s) => WorkoutStep.fromJson(s)).toList() ?? [],
