...
class _HistoryScreenState extends State<HistoryScreen> {
  ...
  Future<void> _loadWorkouts() async {
    try {
      ...
      setState(() {
        ...
        totalDistance = workouts.fold(0.0, (sum, w) => sum + ((w['distance'] ?? 0.0) as num).toDouble());
        totalMinutes = workouts.fold(0, (sum, w) => sum + ((w['duration'] ?? 0) as int));
        isLoading = false;
      });
    } catch (e) {
  ...
  List<Map<String, dynamic>> get filteredWorkouts {
    ...
    if (_selectedFilter == 'Runs') {
      return workouts.where((w) => (w['activity_type'] as String).toLowerCase().contains('run')).toList();
    }
    return workouts.where((w) => !(w['activity_type'] as String).toLowerCase().contains('run')).toList();
  }
                ...
                children: [
                  ...
                  if (filteredWorkouts.isEmpty)
                    ...
                    ...filteredWorkouts.map((workout) => _WorkoutCard(
                      ...
                      distance: '${(workout['distance'] ?? 0.0).toStringAsFixed(1)} km',
