...
import '../services/gait_pathology_analyzer.dart' show GaitPathology, GaitPathologyAnalyzer, Exercise;
import '../services/assessment_report_generator.dart' show AssessmentReport, AssessmentReportGenerator, InjuryRisk, RoadmapPhase;
...
class _AssessmentResultsScreenState extends State<AssessmentResultsScreen>
  ...
  void _analyzeAssessment() async {
    ...
    gaitPathologies = GaitPathologyAnalyzer.analyzeGaitPatterns(widget.assessmentData);
    ...
    report = AssessmentReportGenerator.generateReport(
      ...
      goalStatement: widget.assessmentData['goals'] ?? 'Improve running performance',
    );
    _report = report;
                  ...
                  return Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Expanded(child: Text(injury, style: const TextStyle(fontSize: 14))),
                      ],
                    ),
                  );
                }).toList(),
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 16),
                Expanded(child: _buildExerciseDetail('Frequency', exercise.frequency)),
              ],
            ),
            const SizedBox(height: 8),
            ExpansionTile(
              tilePadding: EdgeInsets.zero,
              title: const Text('Progression Timeline', style: TextStyle(fontSize: 14)),
              children: [
                Text(exercise.progressionTimeline, style: const TextStyle(fontSize: 13)),
              ],
            ),
          ],
        ),
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text('Week Range: ${phase.weekRange}', style: const TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              const Text('Goals:', style: TextStyle(fontWeight: FontWeight.bold)),
              ...phase.goals.map((goal) => Text('• $goal')),
              const SizedBox(height: 12),
              const Text('Training Modifications:', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  void _shareReport() async {
    try {
      ...
      await Share.share(
        reportText,
        subject: 'AISRI Assessment Report - ${DateTime.now().toString().split(' ')[0]}',
      );
    } catch (e) {
      if (mounted) {
  ...
  String _generateShareableReport() {
    ...
    buffer.writeln(_report?.nextStepsRecommendation ?? 'Follow the recommended recovery program');
    buffer.writeln('');
  ...
  void _downloadReport() async {
    try {
                  ...
                  pw.Expanded(
                    ...
                    child: pw.Container(
                      ...
                      child: pw.Stack(
                        children: [
                          pw.Container(
                            width: (entry.value / 100) * 500, // Approximate width
              ...
              ..._gaitPathologies!.map((pathology) => pw.Container(
                ...
                child: pw.Column(
                  ...
                  children: [
                    ...
                    pw.SizedBox(height: 4),
                    pw.Text('Confidence: ${(pathology.confidence * 100).toStringAsFixed(0)}%'),
                  ],
                ),
              )).toList(),
              pw.SizedBox(height: 20),
      ...
      if (kIsWeb) {
        ...
        final file = File('${tempDir.path}/aisri_report_${DateTime.now().millisecondsSinceEpoch}.pdf');
