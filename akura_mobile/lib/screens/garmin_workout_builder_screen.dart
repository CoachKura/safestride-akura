...
class GarminWorkoutBuilderScreen extends StatefulWidget {
  ...
  @override
  State<GarminWorkoutBuilderScreen> createState() => _GarminWorkoutBuilderScreenState();
}

class _GarminWorkoutBuilderScreenState extends State<GarminWorkoutBuilderScreen> {
  ...
  void initState() {
    ...
    if (widget.existingWorkout != null) {
      ...
      _steps = [
        WorkoutStep(id: '${now}_warmup', type: StepType.warmUp, target: StepTarget.open, order: 0),
        WorkoutStep(id: '${now}_run', type: StepType.run, target: StepTarget.distance, targetValue: 1609.34, targetUnit: 'mi', order: 1),
        WorkoutStep(id: '${now}_cooldown', type: StepType.coolDown, target: StepTarget.open, order: 2),
      ];
    }
  }
        ...
        Padding(
          ...
          child: ElevatedButton(
            ...
            child: _isSaving
                ...
                : const Text('Save Workout', style: TextStyle(fontWeight: FontWeight.w600)),
          ),
        ),
      ],
    );
            ...
            child: Row(
              children: [
                ...
                TextButton(
                  ...
                  child: const Text('Edit Step', style: TextStyle(color: Colors.blue, fontSize: 13)),
                ),
                // Delete button
                IconButton(
          ...
          ...step.repeatSteps!.map((s) => Padding(
            ...
            child: Row(
              children: [
                ...
                Text(s.typeName, style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 13)),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    s.targetDescription,
        ...
        InkWell(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 8),
              const Text('Add Note', style: TextStyle(color: Colors.blue, fontSize: 14)),
            ],
          ),
        ),
        const SizedBox(height: 24),
  ...
  String _getTotalEstTime() {
    final total = _steps.fold<Duration>(Duration.zero, (sum, step) => sum + step.estimatedDuration);
  ...
  void _deleteStep(int index) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete "${_steps[index].typeName}"?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              step.typeName,
              style: TextStyle(color: step.typeColor, fontWeight: FontWeight.w500, fontSize: 11),
  ...
  Future<void> _saveWorkout() async {
    if (_steps.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please add at least one step to the workout')),
      );
      return;
    }
    ...
    try {
      final workout = StructuredWorkout(
        ...
        name: _nameController.text.trim().isEmpty ? 'Run Workout' : _nameController.text.trim(),
        steps: _steps,
...
class _StepEditorDialogState extends State<_StepEditorDialog> {
  ...
  void initState() {
    ...
    if (widget.step.targetValue != null) {
      if (widget.step.target == StepTarget.distance) {
        displayValue = widget.step.targetValue! / (_selectedUnit == 'mi' ? 1609.34 : 1000);
      } else if (widget.step.target == StepTarget.duration) {
        displayValue = widget.step.targetValue! / 60; // Convert seconds to minutes
      } else {
      ...
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Target Type', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
              const SizedBox(height: 12),
              Wrap(
                spacing: 8,
                runSpacing: 8,
  ...
  Widget _buildDistanceInput() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Distance', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        const SizedBox(height: 8),
        Row(
          children: [
            Expanded(
  ...
  Widget _buildHRZoneInput() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Heart Rate Zone', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        const SizedBox(height: 12),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [1, 2, 3, 4, 5].map((zone) {
            final isSelected = _selectedHRZone == zone;
            final zoneColors = [Colors.grey, Colors.blue, Colors.green, Colors.orange, Colors.red];
  ...
  String _getZoneDescription(int zone) {
    switch (zone) {
      case 1: return 'Recovery: 50-60% max HR';
      case 2: return 'Endurance: 60-70% max HR';
      case 3: return 'Tempo: 70-80% max HR';
      case 4: return 'Threshold: 80-90% max HR';
      case 5: return 'Max: 90-100% max HR';
      default: return '';
    }
  }
  ...
  Widget _buildPaceInput() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Target Pace Range', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        const SizedBox(height: 8),
        Row(
          children: [
            Expanded(
  ...
  Widget _buildCaloriesInput() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Target Calories', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        const SizedBox(height: 8),
        TextField(
          controller: _valueController,
          keyboardType: TextInputType.number,
  ...
  void _saveStep() {
    ...
    if (_selectedTarget == StepTarget.distance) {
      ...
      targetValue = value * (_selectedUnit == 'mi' ? 1609.34 : (_selectedUnit == 'km' ? 1000 : 1));
  ...
  String _getTargetLabel(StepTarget target) {
    switch (target) {
      case StepTarget.open: return 'Lap Button';
      case StepTarget.distance: return 'Distance';
      case StepTarget.duration: return 'Time';
      case StepTarget.heartRate: return 'HR Zone';
      case StepTarget.pace: return 'Pace';
      case StepTarget.calories: return 'Calories';
    }
  }
}
...
class _RepeatConfigDialogState extends State<_RepeatConfigDialog> {
  ...
  void initState() {
    ...
    final now = DateTime.now().millisecondsSinceEpoch;
    _repeatSteps = [
      WorkoutStep(id: '${now}_interval', type: StepType.interval, target: StepTarget.distance, targetValue: 400, targetUnit: 'm', order: 0),
      WorkoutStep(id: '${now}_recovery', type: StepType.recovery, target: StepTarget.distance, targetValue: 200, targetUnit: 'm', order: 1),
    ];
  }
    ...
    return AlertDialog(
      ...
      content: SizedBox(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Number of Repeats', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            Row(
              children: [
                IconButton(
                  onPressed: _repeatCount > 1 ? () => setState(() => _repeatCount--) : null,
                  icon: Icon(Icons.remove_circle, color: _repeatCount > 1 ? Colors.blue : Colors.grey),
                  iconSize: 32,
                ),
                Container(
                ...
                const Spacer(),
                Text('× repeats', style: TextStyle(color: Colors.grey[600], fontSize: 16)),
              ],
            ),
            const SizedBox(height: 20),
            const Text('Steps to Repeat', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Container(
              decoration: BoxDecoration(
                border: Border.all(color: Colors.grey[300]!),
                ...
                children: [
                  ..._repeatSteps.asMap().entries.map((entry) {
                    ...
                    return ListTile(
                      ...
                      title: Text(step.typeName, style: const TextStyle(fontWeight: FontWeight.w500)),
                      subtitle: Text(step.targetDescription, style: TextStyle(color: Colors.grey[600])),
                      trailing: IconButton(
                        icon: const Icon(Icons.close, size: 18, color: Colors.grey),
                        onPressed: () => setState(() => _repeatSteps.removeAt(idx)),
                      ),
                    );
                  }),
                  const Divider(height: 1),
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Add Step to Repeat', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Wrap(
              spacing: 12,
              runSpacing: 12,
              children: [StepType.run, StepType.interval, StepType.recovery, StepType.walk, StepType.rest].map((type) {
                final step = WorkoutStep(type: type, order: _repeatSteps.length);
                return InkWell(
                  onTap: () {
                    Navigator.pop(context);
                    setState(() {
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Icon(step.typeIcon, color: step.typeColor, size: 28),
                        const SizedBox(height: 4),
                        Text(step.typeName, style: TextStyle(color: step.typeColor, fontSize: 12)),
                      ],
                    ),
                  ),
                );
