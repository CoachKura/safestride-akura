...
class _GPSConnectionScreenState extends State<GPSConnectionScreen> {
            ...
            onPressed: () async {
              ...
              if (code.isNotEmpty) {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Please enter either the full URL or just the code'),
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        if (mounted) {
          ...
          final isLoginError = errorMsg.contains('not logged in') || errorMsg.contains('User not');
          
          showDialog(
            context: context,
            builder: (context) => AlertDialog(
              title: Text(isLoginError ? '⚠️ Login Required' : '❌ Connection Failed'),
              content: SingleChildScrollView(
                child: Column(
                  mainAxisSize: MainAxisSize.min,
                  crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Future<void> _syncActivities() async {
    setState(() {
      _statusMessage = 'Syncing ALL activities from Strava...\n(This may take a few minutes for many activities)';
    });
    ...
    try {
      ...
      setState(() {
        _statusMessage = 'Saving ${activities.length} activities to database...\nOrganizing by calendar date...';
      });

      final saveResult = await _gpsDataFetcher.saveActivitiesToDatabase(activities);
      final savedCount = saveResult['savedCount'] as int;
      final uniqueDates = saveResult['uniqueDates'] as int;
      final activitiesByDate = saveResult['activitiesByDate'] as Map<DateTime, List<dynamic>>;

      setState(() {
        _statusMessage = '✅ Successfully synced ${activities.length} activities!\n'
      ...
      if (mounted && activities.isNotEmpty) {
        showDialog(
          context: context,
          builder: (context) => AlertDialog(
            title: const Text('✅ Activities Synced & Organized by Calendar Date'),
            content: SizedBox(
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('📆 From: ${activities.last.startTime.toString().split(' ')[0]}',
                             style: const TextStyle(fontSize: 11, color: Colors.grey)),
                        Text('📆 To: ${activities.first.startTime.toString().split(' ')[0]}',
                             style: const TextStyle(fontSize: 11, color: Colors.grey)),
                      ],
                    ),
                  ),
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    if (activity.avgCadence != null)
                                      _buildMetricRow('Cadence', '${activity.avgCadence!.round()} spm'),
                                    if (activity.avgStrideLength != null)
                                      _buildMetricRow('Stride Length', '${activity.avgStrideLength!.toStringAsFixed(2)} m'),
                                    if (activity.avgHeartRate != null)
                                      _buildMetricRow('Heart Rate', '${activity.avgHeartRate!.round()} bpm'),
                                    if (activity.avgSpeed != null)
                                      _buildMetricRow('Avg Speed', '${activity.avgSpeed!.toStringAsFixed(1)} km/h'),
                                    if (activity.avgPace != null)
                                      _buildMetricRow('Avg Pace', '${activity.avgPace!.toStringAsFixed(2)} min/km'),
                                    if (activity.elevationGain != null)
                                      _buildMetricRow('Elevation Gain', '${activity.elevationGain!.round()} m'),
                                    if (activity.calories != null)
                                      _buildMetricRow('Calories', '${activity.calories} kcal'),
                                    if (activity.avgWatts != null)
                                      _buildMetricRow('Power', '${activity.avgWatts!.round()} W'),
                                  ],
                                ),
                              ),
                            ],
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_isStravaConnected && _athleteProfile != null) ...[
                            const Divider(height: 32),
                            Row(
                              children: [
                                if (_athleteProfile!['strava_profile_image'] != null)
                                  CircleAvatar(
