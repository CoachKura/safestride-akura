  ...
  Future<void> _loadTodayWorkouts() async {
    try {
      ...
      final response = await Supabase.instance.client
          ...
          .select('id, workout_name, workout_type, duration_minutes, scheduled_date')
          .eq('user_id', userId)
    ...
    try {
      ...
      final activityResponse = await Supabase.instance.client
          ...
          .insert({
            ...
            'athlete_id': userId,
            'platform': 'manual',
            'platform_activity_id': 'manual_${DateTime.now().millisecondsSinceEpoch}',
            'activity_type': _selectedWorkout ?? 'Running',
            'distance_meters': (_distance * 1000).toInt(),
            'duration_seconds': _duration,
            'calories': _calories,
            'avg_speed': _avgSpeed,
            'start_time': DateTime.now().subtract(Duration(seconds: _duration)).toIso8601String(),
        ...
        final trackPointsData = _trackPoints.asMap().entries.map((entry) {
          return {
            ...
            'timestamp': DateTime.now()
                ...
                .add(Duration(seconds: (entry.key * _duration / _trackPoints.length).round()))
                .toIso8601String(),
          };
        }).toList();

        await Supabase.instance.client.from('gps_track_points').insert(trackPointsData);
        developer.log('✅ Activity $activityId saved with ${_trackPoints.length} track points');
      }
    } catch (e) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            _buildDialogStat('Distance', '${_distance.toStringAsFixed(2)} km', Icons.straighten),
            const SizedBox(height: 8),
            _buildDialogStat('Duration', _formatDuration(_duration), Icons.timer),
            const SizedBox(height: 8),
            _buildDialogStat('Avg Pace', _formatPace(_pace), Icons.speed),
            const SizedBox(height: 8),
            _buildDialogStat('Calories', '$_calories kcal', Icons.local_fire_department),
            const SizedBox(height: 8),
            _buildDialogStat('Est. VO2 Max', '${_calculateVO2Max().toStringAsFixed(1)} ml/kg/min', Icons.favorite),
          ],
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text('Go to Calendar tab → Select today → View your workout analysis!'),
                ...
                children: [
                  ...
                  if (_isTracking)
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: _isPaused ? Colors.orange.shade100 : Colors.red.shade100,
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              ...
                              _StatCard(
                                icon: Icons.speed,
                                value: _pace > 0 ? _pace.toStringAsFixed(1) : '--',
                                unit: 'min/km',
                                label: 'Pace',
                              ),
                            ],
