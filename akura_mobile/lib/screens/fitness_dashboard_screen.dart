  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      ...
      final weeklyResponse = await Supabase.instance.client
          .from('gps_activities')
          .select('distance_meters, calories, duration_seconds, start_time, avg_heart_rate')
          .eq('user_id', userId)
  ...
  List<double> _buildMonthlyChart(List<dynamic> activities) {
    ...
    for (var activity in activities) {
      ...
      if (daysAgo >= 0 && daysAgo < 30) {
        chart[29 - daysAgo] += ((activity['distance_meters'] as num?) ?? 0) / 1000;
      }
    }
  ...
  List<double> _buildWeeklySteps(List<dynamic> activities) {
    ...
    for (var activity in activities) {
      ...
      if (daysAgo >= 0 && daysAgo < 7) {
        steps[6 - daysAgo] += ((activity['distance_meters'] as num?) ?? 0) * 1.312;
      }
    }
