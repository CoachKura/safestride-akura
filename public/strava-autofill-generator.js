...
class StravaAutoFillGenerator {
    ...
    async generatePage(athleteData, options = {}) {
        ...
        const filledData = autoFill ? await this.autoFillFields(athleteData, pageType) : {};
    ...
    async autoFillFields(athleteData, pageType) {
        ...
        if (filled.athlete.strava_connected) {
    ...
    async getAthleteInfo(uid) {
        try {
            const response = await fetch(`${this.supabaseUrl}/rest/v1/athletes?uid=eq.${uid}&select=*`, {
                headers: {
    ...
    renderPage(template, data, role) {
        ...
        html = html.replace(/\{\{athlete\.name\}\}/g, data.athlete?.full_name || 'Athlete');
        ...
        // Strava data
        html = html.replace(/\{\{strava\.username\}\}/g, data.strava?.athlete_data?.username || 'Not connected');
        html = html.replace(/\{\{strava\.profile_url\}\}/g, data.strava?.athlete_data?.profile || '#');
        html = html.replace(/\{\{strava\.avatar\}\}/g, data.strava?.athlete_data?.profile_medium || '/assets/default-avatar.png');
        
        // AISRI scores
        html = html.replace(/\{\{aisri\.total\}\}/g, data.aisri?.total_score || 0);
        html = html.replace(/\{\{aisri\.risk\}\}/g, data.aisri?.risk_category || 'Unknown');
        html = html.replace(/\{\{aisri\.running\}\}/g, data.aisri?.pillar_scores?.running || 0);
        html = html.replace(/\{\{aisri\.strength\}\}/g, data.aisri?.pillar_scores?.strength || 0);
        html = html.replace(/\{\{aisri\.rom\}\}/g, data.aisri?.pillar_scores?.rom || 0);
        html = html.replace(/\{\{aisri\.balance\}\}/g, data.aisri?.pillar_scores?.balance || 0);
        html = html.replace(/\{\{aisri\.alignment\}\}/g, data.aisri?.pillar_scores?.alignment || 0);
        html = html.replace(/\{\{aisri\.mobility\}\}/g, data.aisri?.pillar_scores?.mobility || 0);
        
        // Computed fields
        html = html.replace(/\{\{computed\.totalActivities\}\}/g, data.computed?.totalActivities || 0);
        html = html.replace(/\{\{computed\.totalDistance\}\}/g, (data.computed?.totalDistance || 0).toFixed(1));
        html = html.replace(/\{\{computed\.totalTime\}\}/g, Math.round(data.computed?.totalTime || 0));
        html = html.replace(/\{\{computed\.averagePace\}\}/g, this.formatPace(data.computed?.averagePace || 0));
        html = html.replace(/\{\{computed\.recentForm\}\}/g, data.computed?.recentForm || 'Unknown');
        html = html.replace(/\{\{computed\.riskLevel\}\}/g, data.computed?.riskLevel || 'Unknown');
...
<body class="bg-gray-50">
    ...
    <script>
        ...
        async function syncStrava() {
            ...
            try {
                const response = await fetch('https://your-project.supabase.co/functions/v1/strava-sync-activities', {
                    method: 'POST',
                    headers: {
